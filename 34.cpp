...
vvi comp;
vvi zone;
void dfs_comp(int x, int y, int n){
    comp[x][y] = n;
    int k = zone[x][y];
    
    if (x) 
        if ((comp[x - 1][y] == -1) && (zone[x - 1][y] == k)) 
            dfs_comp(x - 1, y, n);
    if (y) 
        if ((comp[x][y - 1] == -1) && (zone[x][y - 1] == k)) 
            dfs_comp(x, y - 1, n);
    if (x < comp.size() - 1) 
        if ((comp[x + 1][y] == -1) && (zone[x + 1][y] == k)) 
            dfs_comp(x + 1, y, n);
...
inline void solve(){
    ...
    comp.assign(n, vi(m, -1));
    zone.assign(n, vi(m, 0));
    rep(i, n) rep(j, m) cin >> zone[i][j];
    
    int k = 0;
    rep(i, n) rep(j, m) if (comp[i][j] == -1) dfs_comp(i, j, k++);
    //rep(i, n){rep(j, m) cout << comp[i][j] << " "; cout << '\n';}
    
    
    graph.resize(k);
    rep(i, n) rep_1(j, m){
        if (zone[i][j - 1] < zone[i][j]) graph[comp[i][j]].push_back(comp[i][j - 1]);
        if (zone[i][j - 1] > zone[i][j]) graph[comp[i][j - 1]].push_back(comp[i][j]);
    } 
    rep_1(i, n) rep(j, m){
        if (zone[i - 1][j] < zone[i][j]) graph[comp[i][j]].push_back(comp[i - 1][j]);
        if (zone[i - 1][j] > zone[i][j]) graph[comp[i - 1][j]].push_back(comp[i][j]);
    } 
    
    size_t res = 0;
    //rep(i, k) cout << gr 
    rep(i, k) if (!graph[i].size()) res++;
    cout << res;
}
